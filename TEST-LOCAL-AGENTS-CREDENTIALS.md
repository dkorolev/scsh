...
6. **Casts recorded, chapters annotated.** `ls -t tmp/casts/harness-smoke-creds-*.cast`
   and, for each fresh cast, check its sidecar: `ls <cast-basename>.chapters.json`
   (e.g. `tmp/casts/harness-smoke-creds-codex-gpt-5.5-<stamp>-utc-<nonce>.chapters.json`).
