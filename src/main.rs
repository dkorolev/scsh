...
/// error document (packdiff 0.4.1). Progress stays on stderr and is discarded.
fn pack_step_diff(
  ...
  match result {
    ...
    Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
      hint(
        "packdiff not found — `cargo install packdiff --version 0.4.1 --locked` to browse each step's commits from the job page",
      );
    }
