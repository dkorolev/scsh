...
**Output.** Write your result to the path in `$SCSH_RESULT` when running under scsh (each model route declares its own `result:` in `.scsh.yml`), otherwise `tmp/code-review-reviewability-reviewer.json` when invoked on its own — a single JSON object of this shape:
